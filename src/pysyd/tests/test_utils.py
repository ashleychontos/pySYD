...
def test_targets():
    ...
    try:
        params.add_targets()              # this will raise utils.InputError
    except InputError:
        assert params.params['stars'] is None, "Where are stars being provided"
    chuck = params.params.pop('stars')
    params.add_targets(stars=[1435467,2309595,11618103])
    assert params.params['stars'] is not None, "Not loading in target parameters"
