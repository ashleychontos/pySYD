...
def plot_background(target):
    ...
    ax3.plot(target.frequency, target.power, c='lightgrey', zorder=0, alpha=0.5)
    ax3.plot(target.frequency[target.frequency < target.maxpower[0]], target.power[target.frequency < target.maxpower[0]], 'w-', linewidth=0.75, zorder=1)
    ax3.plot(target.frequency[target.frequency > target.maxpower[1]], target.power[target.frequency > target.maxpower[1]], 'w-', linewidth=0.75, zorder=1)
    ax3.plot(target.frequency[target.frequency < target.maxpower[0]], target.smooth_power[target.frequency < target.maxpower[0]], 'r-', linewidth=0.75, zorder=2)
    ax3.plot(target.frequency[target.frequency > target.maxpower[1]], target.smooth_power[target.frequency > target.maxpower[1]], 'r-', linewidth=0.75, zorder=2)
    for r in range(target.nlaws):
        ax3.plot(target.frequency, harvey(target.frequency, [target.pars[2*r], target.pars[2*r+1], target.pars[-1]]), color='blue', linestyle=':', linewidth=1.5, zorder=4)
    ax3.plot(target.frequency, harvey(target.frequency, target.pars, total=True), color='blue', linewidth=2.0, zorder=5)
    ax3.errorbar(target.bin_freq, target.bin_pow, yerr=target.bin_err, color='lime', markersize=0.0, fillstyle='none', ls='None', marker='D', capsize=3, ecolor='lime', elinewidth=1, capthick=2, zorder=3)
    ...
    ax3.plot(target.frequency, target.pssm, color='yellow', linewidth=2.0, linestyle='dashed', zorder=6)
    ...
    sig = 0.35*target.exp_dnu/2.35482 
    weights = 1./(sig*np.sqrt(2.*np.pi))*np.exp(-(target.lag-target.exp_dnu)**2./(2.*sig**2))
    ...
    ax6.axvline(target.exp_dnu, color='red', linestyle=':', linewidth=1.5, zorder=5)
    ...
    ax6.plot(target.lag, new_weights, c='yellow', linestyle=':', zorder = 0, linewidth = 1.0)
    ...
    ax7.axvline(target.obs_dnu, color='lime', linestyle='--', linewidth=1.5, zorder=2)
    ax7.plot(target.new_lag, target.dnu_fit, color='lime', linewidth=1.5)
    ax7.axvline(target.exp_dnu, color='red', linestyle=':', linewidth=1.5, zorder=5)
