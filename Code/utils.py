...
class PowerSpectrum:
    ...
    def __init__(self, findex, fitbg, verbose, correct = False):
        self.findex = {}
...
# 1) Ability to add more trials for numax determination
    ...
    def find_excess(self):
        ...
        for target in self.params['todo']:
            ...
            print()
            if not os.path.exists(self.params['path']+'%d_LC.txt'%target):
                if self.verbose:
                    print('Error: %s%d_LC.txt not found'%(self.params['path'], target))
                break
            else:
                self.get_file(self.params['path']+'%d_LC.txt'%target)
                self.time = np.copy(self.x)
                self.flux = np.copy(self.y)
                if self.verbose:
                    print('# LIGHT CURVE: %d lines of data read'%len(self.time))

            if not os.path.exists(self.params['path']+'%d_PS.txt'%target):
                if self.verbose:
                    print('Error: %s%d_PS.txt not found'%(self.params['path'], target))
                break
            else:
                self.get_file(self.params['path']+'%d_PS.txt'%target)
            ...
            if self.findex['lower_limit'] is not None:
                if self.findex['upper_limit'] is not None:
                    self.mask = np.ma.getmask(np.ma.masked_inside(self.frequency, self.findex['lower_limit'], self.findex['upper_limit']))
                else:
                    self.mask = np.ma.getmask(np.ma.masked_greater_equal(self.frequency, self.findex['lower_limit']))
            else:
                if self.findex['upper_limit'] is not None:
                    self.mask = np.ma.getmask(np.ma.masked_less_equal(self.frequency, self.findex['upper_limit']))
                else:
                    self.mask = np.ones_like(self.frequency)
            self.frequency = self.frequency[self.mask]
            self.power = self.power[self.mask]
            ...
            if self.findex['binning'] is not None:
                ...
                for i, box in enumerate(boxes):
                    ...
                    while True:
                        ...
                        lag = np.arange(len(p))*self.resolution
                        auto = np.correlate(p-np.mean(p), p-np.mean(p), "full")    
                ...
                self.write_excess(target, results[best])

            if self.findex['plot']:
                self.plot_findex(target, nrows)


    def plot_findex(self, target, nrows):

        plt.figure(figsize = (12,8))
        
        ax1 = plt.subplot(1+nrows,3,1)
        ...
        ax2.loglog(self.frequency, self.interp_pow, color = 'lime', linestyle = '-', lw = 2.)

        ax3 = plt.subplot(1+nrows,3,3)
        ax3.plot(self.frequency, self.bgcorr_pow, 'w-')
        ax3.set_xlim([min(self.frequency), max(self.frequency)])
        ax3.set_ylim([min(self.bgcorr_pow), max(self.bgcorr_pow)*1.25])
        ...
        plt.tight_layout()
        if self.findex['save']:
            plt.savefig(self.params[target]['path']+'%d_findex.png'%target, dpi = 300)
        plt.show()
        plt.close()
...
#                    else:
                ...
                # sampling process
                nlaws = self.fitbg['n_laws']
                for i in range(self.fitbg['num_mc_iter']):
                    self.i = i
                    if i == 0:
                    ...
                    self.params[target]['mask'] = (self.original_freq>=self.maxpower[0])&(self.original_freq<=self.maxpower[1])
                    outer_x = self.original_freq[~self.params[target]['mask']]
                    outer_y = random_pow[~self.params[target]['mask']]
                    ...
                    if i == 0:
                        ...
                        if self.verbose:
                            print('Comparing %d different models:'%(self.fitbg['n_laws']*2))
                        # get best fit model 
                        bounds = []
                        for law in range(self.fitbg['n_laws']):
                        ...
                        dict1 = dict(zip(np.arange(2*self.fitbg['n_laws']),names[:2*self.fitbg['n_laws']]))
                        for t in range(2*self.fitbg['n_laws']):
                            if t%2 == 0:
                                if self.verbose:
                                    print('%d: %s harvey model w/ white noise free parameter'%(t+1, dict1[t]))
                                delta = 2*(self.fitbg['n_laws']-(t//2+1))
                        ...
                        self.pars = pars
                        final_pars = np.zeros((self.fitbg['num_mc_iter'],nlaws*2+12))

                        self.sm_par = 4.*(self.params[target]['numax']/self.params['numax_sun'])**0.2
                        if self.sm_par < 1.:
                            self.sm_par = 1.
                    else:
                    ...
                    x0 = list(self.original_freq[self.params[target]['mask']])
                    t0 = pssm[self.params[target]['mask']]
                    t1 = model[self.params[target]['mask']]
                    delta_y = t0[-1] - t0[0]
                    delta_x = x0[-1] - x0[0]
                    ...
                    x2 = list(self.original_freq[~self.params[target]['mask']])
                    t2 = list(model[~self.params[target]['mask']])
                    ...
                    region_freq = np.copy(self.original_freq[self.params[target]['mask']])
                    region_pow = pssm_bgcorr[self.params[target]['mask']]
                    idx = self.return_max(region_pow, index=True)
                    final_pars[i,2*nlaws+1] = region_freq[idx]
                    final_pars[i,2*nlaws+2] = region_pow[idx]

                    if region_freq != []:
                        bounds = self.gaussian_bounds(region_freq, region_pow)
                        guesses = [0., max(region_pow), region_freq[idx], (max(region_freq)-min(region_freq))/8./np.sqrt(8.*np.log(2.))]
                        p_gauss1, p_cov = curve_fit(gaussian, region_freq, region_pow, p0 = guesses, bounds = bounds[0])
                        final_pars[i,2*nlaws+3] = p_gauss1[1]
                        final_pars[i,2*nlaws+4] = p_gauss1[2]
                        final_pars[i,2*nlaws+5] = p_gauss1[3]
                    ...
                    self.sm_par = 4.*(self.numax_exp/self.params['numax_sun'])**0.2
                    self.dnu_exp = 0.22*(p_gauss1[2]**0.797)
                    self.times = self.width/self.dnu_exp
                    lim_factor = self.times*self.dnu_exp
                    if self.fitbg['lower_numax'] is not None:
                        msk = (self.original_freq >= self.fitbg['lower_numax'])&(self.original_freq <= self.fitbg['upper_numax'])
                    else:
                        msk = (self.original_freq >= p_gauss1[2]-lim_factor)&(self.original_freq <= p_gauss1[2]+lim_factor)
                    ...
                    bounds = self.gaussian_bounds(lag, auto, best_x = best_lag, sigma = 10**-2)
                    guesses = [np.mean(auto), best_auto, best_lag, best_lag*0.01*2.]
                    p_gauss2, p_cov2 = curve_fit(gaussian, lag, auto, p0 = guesses, bounds = bounds[0])

                    mask = (lag >= best_lag-2.*p_gauss2[3]*np.sqrt(8.*np.log(2.)))&(lag <= best_lag+2.*p_gauss2[3]*np.sqrt(8.*np.log(2.)))
                    zoom_lag = lag[mask]
                    zoom_auto = auto[mask]
                    fit = gaussian(zoom_lag, p_gauss2[0], p_gauss2[1], p_gauss2[2], p_gauss2[3])
                    idx = self.return_max(fit, index=True)
                    final_pars[i,2*nlaws+6] = zoom_lag[idx]
    ...
    def load_data(self):
        ...
        if os.path.exists(self.params[self.target]['path']+'%d_findex.txt'%self.target):
            f = open(self.params[self.target]['path']+'%d_findex.txt'%self.target)
            lines = f.readlines()
            line = lines[1]
            for i, var in enumerate(['numax', 'dnu', 'snr']):
                self.params[self.target][var] = float(line.strip().split()[i+1])
            ...
            for i in range(len(gridx)):
                for j in range(len(gridy)):
                    ...
                    if len(use) == 0:
                        ...
                        arr[i,j] = np.sum(self.bg_corr[use])
                    gridy[j] = starty + self.dnu/2.
                    starty += self.dnu
                gridx[i] = startx + self.dnu/n_across/2.
                starty = min(self.original_freq)
...
#def confirm_excess(frequency, power, numax, target, params):
    ...
    def plot_fitbg(self):
        ...
        peaks_l, peaks_a = self.max_elements(list(self.lag), list(self.auto), limit = [True, 20.*self.resolution])
        idx0 = self.return_max(peaks_a, index=True)
        ax6 = fig.add_subplot(3,3,6)
        ax6.plot(self.lag, self.auto, 'w-', zorder = 0, linewidth = 1.)
        ax6.scatter(peaks_l[:idx0], peaks_a[:idx0], s = 25., edgecolor = 'r', marker = '^', facecolor = 'none', linewidths = 1.)
        ax6.scatter(peaks_l[idx0+1:], peaks_a[idx0+1:], s = 25., edgecolor = 'r', marker = '^', facecolor = 'none', linewidths = 1.)
        ax6.axvline(peaks_l[idx0], color = 'red', linestyle = '--', linewidth = 1.5, zorder = 2)
        ax6.scatter(peaks_l[idx0], peaks_a[idx0], s = 45., edgecolor = 'lime', marker = 's', facecolor = 'none', linewidths = 1.)
        ...
        idx1 = self.return_max(fit, index=True)
        ...
        ax7.axvline([peaks_l[idx0]], color = 'red', linestyle = '--', linewidth = 1.5, zorder = 2)
        ax7.plot(self.zoom_lag, fit, color = 'lime', linewidth = 1.5)
        ax7.axvline([self.zoom_lag[idx1]], color = 'lime', linestyle = '--', linewidth = 1.5)
        ax7.set_title(r'$\rm \Delta\nu \,\, fit$')
        ax7.set_xlabel(r'$\rm Frequency \,\, separation \,\, [\mu Hz]$')
        ax7.annotate(r'$\Delta\nu = %.2f$'%self.zoom_lag[idx1], xy = (0.025, 0.85), xycoords = "axes fraction", fontsize = 18, color = 'lime')
...
#        ax9.set_ylim([min(self.yax), max(self.yax)])
...
##########################################################################################
    ...
    def return_max(self, array, index=False, dnu=False):
        ...
        if dnu:
            lst = list(np.absolute(np.copy(array)-self.dnu_exp))
